...
\begin{equation}(\displaystyle\sum_{s=1}^t \sigma_s){\mathbf{g}_{t+1}} = \displaystyle\sum\_{s=1}^t {\sigma_s(\mathbf{w}_s} - \displaystyle\sum\_{s=1}^t {\mathbf{g}_s}\end{equation}
